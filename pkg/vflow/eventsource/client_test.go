...
import (
	...
	"github.com/skupperproject/skupper/pkg/session"
	"github.com/skupperproject/skupper/pkg/vflow"
	"gotest.tools/assert"
)
...
func TestClient(t *testing.T) {
	...
	heartbeats := make(chan vflow.HeartbeatMessage, 8)
	records := make(chan vflow.RecordMessage, 8)
	client.OnHeartbeat(func(m vflow.HeartbeatMessage) { heartbeats <- m })
	client.OnRecord(func(m vflow.RecordMessage) { records <- m })

	sender := tstCtr.NewSender(mcsfe(clientID), session.SenderOptions{})
	assert.Check(t, client.Listen(tstCtx, FromSourceAddress()))

	heartbeat := vflow.HeartbeatMessage{
		Identity: clientID, Version: 1, Now: 22,
		MessageProps: vflow.MessageProps{
			To:      mcsfe(clientID),
			Subject: "HEARTBEAT",
		},
	}
	...
	for i := 0; i < 10; i++ {
		...
		record.Records = append(record.Records, &vflow.RouterRecord{BaseRecord: vflow.BaseRecord{ID: name}})
	}
...
func sendHeartbeatMessagesTo(t *testing.T, ctx context.Context, sender session.Sender) {
	...
	for {
		...
		case <-time.After(time.Millisecond):
			err := sender.Send(ctx, vflow.HeartbeatMessage{}.Encode())
			if ctx.Err() != nil {
				return
			}
			assert.Check(t, err)
...
func requireContainers(t *testing.T) session.ContainerFactory {
	...
	var (
		...
		errNotSet = errors.New("TEST_ROUTER_ADDRESS environment variable not present")
	)
	err := func() error {
		qdr := os.Getenv("TEST_ROUTER_ADDRESS")
		if qdr == "" {
			return errNotSet
		}
