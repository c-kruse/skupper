...
type HeartbeatMessageHandler func(vflow.HeartbeatMessage)
type RecordMessageHandler func(vflow.RecordMessage)
...
func (c *Client) Listen(ctx context.Context, attributes ListenerConfigProvider) error {
	...
	go func(ctx context.Context) {
		...
		for {
			...
			decoded, err := vflow.Decode(amqpMsg)
			if err != nil {
			...
			case vflow.RecordMessage:
				for _, handler := range recordHandlers {
					handler(message)
				}
			case vflow.HeartbeatMessage:
