...
func (d *Discovery) Run(ctx context.Context, handlers DiscoveryHandlers) error {
	...
	for {
		...
		d.observe(vflow.DecodeBeacon(msg))
	}
}
...
func newWatch(discovery *Discovery, client *Client) *watch {
	...
	client.OnHeartbeat(func(vflow.HeartbeatMessage) {
		ts := time.Now()
		w.lastSeen.Store(&ts)
	})

	client.OnRecord(func(vflow.RecordMessage) {
		ts := time.Now()
		w.lastSeen.Store(&ts)
	})
	return &w
...
func (d *Discovery) observe(beacon vflow.BeaconMessage) {
	var (
		state      Info
		discovered bool
	)
