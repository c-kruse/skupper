...
import (
	...
	"github.com/skupperproject/skupper/pkg/vflow/encoding"
	"gotest.tools/assert"
)
...
func TestMarshal(t *testing.T) {
	timeA := vflow.Time{Time: time.UnixMicro(100)}
	timeB := vflow.Time{Time: time.UnixMicro(333)}
	testCases := []struct {
		Name          string
		Input         interface{}
		ExpectErr     bool
	...
	}{
		{
			...
			Name:  "basic site",
			Input: vflow.SiteRecord{BaseRecord: vflow.BaseRecord{ID: "test"}},
			...
			Input:     vflow.SiteRecord{BaseRecord: vflow.NewBase("test", time.UnixMicro(-1))},
			ExpectErr: true,
		},
	}
