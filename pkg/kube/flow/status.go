...
func (s *StatusSync) Run(ctx context.Context) {
	...
	for {
		...
		case source := <-s.purgeQueue:
			ct := s.purge(source)
			slog.Info("purged records from forgotten source",
...
func (s *StatusSync) build() network.NetworkStatusInfo {
	...
	sorted := func(s []store.Entry) []store.Entry {
		slices.SortFunc(s, func(a, b store.Entry) int {
		...
		for _, routerEnt := range routerEntries {
			...
			for i, connEnt := range connEntries {
				...
				if !matchedProc {
					...
					procs := s.records.Index(byHost, procExemplar)
					if len(procs) > 0 {
...
func (s *StatusSync) purge(source store.SourceRef) int {
	matching := s.records.Index(store.TypeIndex, store.Entry{Metadata: store.Metadata{Source: source}})
