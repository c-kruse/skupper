...
func (e typeEncoding) decode(attrsV reflect.Value, root reflect.Value) error {
	var v reflect.Value
	for _, field := range e.fields {
		objV := attrsV.MapIndex(reflect.ValueOf(field.Codepoint))
		if !objV.IsValid() {
			if field.Identity {
		...
		err := field.decoder.decode(objV.Interface(), v)
		if err != nil {
