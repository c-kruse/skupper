...
`, os.Args[0], os.Args[0])
	...
	flag.StringVar(&Package, "package", "v1", "name of go package to generate")
}
...
func Parse(root Root) []RecordTypeInfo {
	...
	for name, spec := range root.Spec {
		code := spec.(map[string]any)["code"].(int)
		var attributes []RecordAttributeInfo
		for _, attrSpec := range spec.(map[string]any)["attributes"].([]any) {
			attrSpecMap := attrSpec.(map[string]any)
