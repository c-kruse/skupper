	...
	testCases := []struct {
	...
	}{
		{
			...
			In: func() *amqp.Message {
				m, err := RecordMessage{
					Records: []any{
						SiteRecord{BaseRecord: BaseRecord{Identity: "1"}},
						&RouterRecord{BaseRecord: BaseRecord{Identity: "1"}},
						LinkRecord{
							BaseRecord: BaseRecord{Identity: "1"},
							LinkCost:   func() *uint64 { var cost uint64 = 1; return &cost }(),
						},
					},
				}.Encode()
				if err != nil {
			...
			Name: "records in map of any",
			In: func() *amqp.Message {
				m, err := RecordMessage{
					Records: []any{SiteRecord{BaseRecord: BaseRecord{Identity: "1"}}},
				}.Encode()
				if err != nil {
					t.Fatal(err)
				}
				lst := m.Value.([]interface{})
				prev := lst[0].(map[uint32]any)
				dupe := make(map[any]any, len(prev))
				for k, v := range prev {
					dupe[k] = v
				}
				lst[0] = dupe
