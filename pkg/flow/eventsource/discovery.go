...
func NewDiscovery(factory messaging.ConnectionFactory) *Discovery {
	return &Discovery{
		factory:    factory,
	...
	for {
		...
		case msg, ok := <-beaconMsgs:
			...
			if msg.Properties.Subject != "BEACON" {
				slog.Info("received non-beacon from beacon source", slog.String("subject", msg.Properties.Subject), slog.String("source", msg.Properties.To))
				continue
			}
			d.observe(v1.DecodeBeacon(msg))
...
func newWatch(discovery *Discovery, client *Client) *watch {
	...
	client.OnHeartbeat(func(v1.HeartbeatMessage) {
		ts := time.Now()
		w.lastSeen.Store(&ts)
	})

	client.OnRecord(func(v1.RecordMessage) {
		ts := time.Now()
		w.lastSeen.Store(&ts)
	})
	return &w
...
func (d *Discovery) observe(beacon v1.BeaconMessage) {
	var (
		state      Info
		discovered bool
	)
