...
type Manager struct {
	...
	sendQueue   chan v1.RecordMessage
}

func NewManager(factory messaging.ConnectionFactory, cfg ManagerConfig) *Manager {
	return &Manager{
		factory:       factory,
		ManagerConfig: cfg,
		flushQueue:    make(chan struct{}, 8),
		changeQueue:   make(chan RecordUpdate, 256),
		sendQueue:     make(chan v1.RecordMessage, 256),
	}
}
...
func (m *Manager) sendRecords(ctx context.Context) {
	utils.RetryWithContext(ctx, time.Second, func() (bool, error) {
		conn, err := m.factory.Connect()
		if err != nil {
			slog.Error("could not establish connection for event source records", slog.Any("error", err))
			return false, nil
		}
		sender, err := conn.Sender(m.Source.Address)
		if err != nil {
...
func diffRecord(d RecordUpdate) (v1.Record, bool) {
	if d.Prev == nil {
		return d.Curr, true
	}
	...
	return deltaRecord.(v1.Record), true
}
...
func (m *Manager) serve(ctx context.Context) {
	for {
		...
		case update := <-m.changeQueue:
			...
			var record v1.RecordMessage
			record.Records = make([]v1.Record, 0, len(buffer))
		...
		case <-m.flushQueue:
			...
			for len(flushStoreQ) > 0 {
				...
				var record v1.RecordMessage
				record.Records = make([]v1.Record, len(result.Entries))
...
func (m *Manager) sendKeepalives(ctx context.Context) {
	...
	beaconMessage := v1.BeaconMessage{
	...
	heartbeatMessage := v1.HeartbeatMessage{
		Version:  uint32(m.Source.Version),
		Identity: m.Source.ID,
	}
	...
	utils.RetryWithContext(ctx, time.Second, func() (bool, error) {
		...
		defer beaconSender.Close()
		heartbeatSender, err := conn.Sender(heartbeatAddr)
		if err != nil {
			slog.Error("could not open sender for event source heartbeat", slog.Any("error", err))
			return false, nil
		}
		defer heartbeatSender.Close()
...
func sendWithTimeout(ctx context.Context, timeout time.Duration, sender messaging.Sender, msg *amqp.Message) error {
	...
	defer cancel()
	err := sender.SendContext(ctx, msg)
	if err != nil {
