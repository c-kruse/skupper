...
func TestCacheAccumulate(t *testing.T) {
	...
	pointsInTime := []struct {
	...
	}{
		{
			...
			Desc: "record 1",
			ToAccumulate: &v1.SiteRecord{
				BaseRecord: v1.NewBase("1", time.UnixMicro(100)),
				Location:   ptrTo("location 1"),
			},
			ExpectState: func(t *testing.T, entries []Entry) {
				assert.Assert(t, len(entries) == 1)
				assert.DeepEqual(t, entries[0].Record, &v1.SiteRecord{
					BaseRecord: v1.NewBase("1", time.UnixMicro(100)),
					Location:   ptrTo("location 1"),
				})
			},
			...
			Desc: "record 2",
			ToAccumulate: &v1.SiteRecord{
				BaseRecord: v1.NewBase("2", time.UnixMicro(0)),
			},
			ExpectState: func(t *testing.T, entries []Entry) {
			...
			ExpectState: func(t *testing.T, entries []Entry) {
				assert.Assert(t, len(entries) == 2)
				if entries[0].Record.(*v1.SiteRecord).BaseRecord.ID != "1" {
			...
			ToAccumulate: &v1.SiteRecord{
				BaseRecord: v1.NewBase("2", time.UnixMicro(0), time.UnixMicro(9000)),
			},
			ExpectState: func(t *testing.T, entries []Entry) {
				assert.Assert(t, len(entries) == 2)
				if entries[0].Record.(*v1.SiteRecord).BaseRecord.ID != "2" {
					entries[0], entries[1] = entries[1], entries[0]
				}
				assert.DeepEqual(t, entries[0].Record, &v1.SiteRecord{
					BaseRecord: v1.NewBase("2", time.UnixMicro(0), time.UnixMicro(9000)),
	...
	entries := []Entry{
		{
			...
			Record: v1.SiteRecord{BaseRecord: v1.NewBase("1")},
		},
		{
			Meta: Metadata{
				Source: source,
			},
			Record: v1.SiteRecord{BaseRecord: v1.NewBase("2", time.UnixMicro(0))},
		},
	}
...
func TestList(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		entries = append(entries, Entry{
			...
			Record: v1.SiteRecord{BaseRecord: v1.NewBase(fmt.Sprint(i))},
		})
	}
...
func TestConditionalUpdate(t *testing.T) {
	...
	baseEntry := Entry{Record: v1.LinkRecord{BaseRecord: v1.NewBase("A")}}
