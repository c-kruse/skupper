// session implements amqp connection and session management though an
// interface inspired by the Conatiner interface exposed by the qpid proton
// amqp libraries.
//
// The goal of this design differs from that of the messaging interface
// implemented in pkg/qdr as its intended use is for long lived connections as
// opposed to brief call and respose exchanges with the skupper router management
// endpoint.
package session
...
// session when a link encounters an error using a chosen backoff strategy.
