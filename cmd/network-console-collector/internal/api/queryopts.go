...
func filterAndOrderResults[T any](r *http.Request, results []T) ([]T, BaseResponse) {
	...
	for path := range qp.FilterFields {
		...
		if err != nil {
			problems = append(problems, fmt.Sprintf(
				"unexpected filter parameter %q on result type %T",
				path, results))
			continue
		}
		filterFields[path] = m
	...
	if err != nil {
		problems = append(problems, fmt.Sprintf(
			"malformed sortBy parameter %q: err: %s",
			r.URL.Query().Get("sortBy"), err))
		resposne.Status = strings.Join(problems, "\n")
		return out[:0], resposne
	}
