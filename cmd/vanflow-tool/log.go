...
func logOnly(ctx context.Context, factory messaging.ConnectionFactory) {
	outputstream := make(chan Message, 64)

	heartbeatHandler := func(m v1.HeartbeatMessage) {
	...
	recordHandler := func(m v1.RecordMessage) {
	...
	go discovery.Run(ctx, eventsource.DiscoveryHandlers{
		Discovered: func(source eventsource.Info) {
			...
			if FlushOnDiscover {
				...
				client.OnRecord(func(_ v1.RecordMessage) {
					flushOnce()
				})
			}
