...
func TestBasic(t *testing.T) {
	...
	handler, err := newBasicAuthHandler(tmpDir)
	if err != nil {
		t.Fatal("unexpected error", err)
	}

	tstSrv := httptest.NewTLSServer(handler.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		rw.Write([]byte("OK"))
	}))
	defer tstSrv.Close()
	...
	mixedUserPass.SetBasicAuth("skupper", configuredUsers["test-user"])
...
func FuzzBasic(f *testing.F) {
	...
	basic := basicAuthHandler{
		sha256.Sum256([]byte(tUser)): sha256.Sum256([]byte(tPassword)),
	}
