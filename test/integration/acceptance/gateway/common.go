...
func Setup(stopCh chan interface{}, testRunner base.ClusterTestRunner) error {
	...
	dep, err := pub.VanClient.KubeClient.AppsV1().Deployments(pub.Namespace).Create(ctx, tcp_echo.Deployment, v1.CreateOptions{})
	if err != nil {
	...
	if err := pub.VanClient.ServiceInterfaceBind(ctx, tcpEchoClusterSvc, "deployment", dep.Name, map[int]int{9090: 9090}, ""); err != nil {
...
func TearDown(testRunner base.ClusterTestRunner) error {
	...
	if err := pub.VanClient.KubeClient.AppsV1().Deployments(pub.Namespace).Delete(context.TODO(), tcp_echo.Deployment.Name, v1.DeleteOptions{}); err != nil {
		return err
	}
