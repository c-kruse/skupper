...
`, os.Args[0])
	...
	flags.BoolVar(&IstioSource, "send-istio-context", false, "publishes istio context to other collectors in the VAN")
...
func run(factory session.ContainerFactory) error {
	...
	collector, err := newFlowCollector(factory)
	if err != nil {
		return err
	}
	...
	if IstioSource {
		...
		go func() {
			defer wg.Done()
			// TODO
			slog.Error("istio source not implemented")
		}()
	}
	<-ctx.Done()
