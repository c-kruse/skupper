...
of all migration phases. The prerequisites `compressed-router-config` and
`add-skrouter-management-client` are merged into this checkout locally.
...
The isolated cluster is `local-first`, context `kind-local-first`. It contains
...
Automated checks: `go test -short ./...`, targeted router-status/controller/site
tests, publisher conflict/no-op/cancellation tests, and race checks of the new
publisher and network index. See the thread for the final executed check results.
