...
New package `internal/routerstatus` (platform neutral, uses the existing
`internal/qdr` configuration model and management agent).
...
| `exposePodsByName` targets | Prefix scan of `router.address` for `M<routingKey>.`. Interior routers hold the full mobile address table (mobile_sync). Edge routers do **not** (`addr_proxy.c` only proxies addresses with local interest), so on an edge router the adaptor issues the query to its uplink interior via `_topo/0/<interior>/$management` using `qdr.GetRouterAgentAddress`, where the interior id is the `container` of the open `dir=out, role=edge` connection. |
| Sites in network | vanflow SITE records. |
| Configuration applied | The adaptor's own `ConfigSync` knows the ConfigMap resourceVersion it last applied and the error, if any. Presence per entity comes from queries through `qdr.Agent`. |
...
The router rejects unknown sections in `skrouterd.json`, so adaptor-only
configuration cannot live inside the router document. Add a second key to the router
...
The compressed-router-configuration support is already on `origin/main`:
`kubeqdr.ConfigMapWriter`, `GetRouterConfigFromConfigMap`, and ADR 0011.

Both config sync and status use `internal/qdr`. Config sync keeps its existing
`qdr.AgentPool`; the status publisher owns a separate `qdr.Agent` connected to
`amqp://localhost:5672` and reconnects after query failures. Desired configuration
is parsed with `qdr.UnmarshalRouterConfig`. The agent's context-aware query API
adds pagination and cancellation while retaining the five-second query timeout.
No new router client library or generic reconciliation package is required.
...
* Add `internal/routerstatus/document.go` with the types above and
...
* `status.Builder.Build(ctx, desired *qdr.RouterConfig,
  ...
  `router.address` (paged, `qdr.Agent.QueryByAgentAddressContext`). For edge mode, prefix scans and
  address reachability for *non-referenced* addresses are issued to
  `qdr.GetRouterAgentAddress(uplinkID, false)`; uplink discovered from the open edge
  `connection.container`.
* `SiteIndex` (`internal/routerstatus/siteindex.go`): a `store.Interface`
...
* `internal/routerstatus/build_test.go` against a fake query client
  implementing the agent's query interface, covering: listener up/down,
...
* Integration: extend `tests/integration/kube/controller` suite (envtest +
  real router queried through `qdr.Agent`) to assert the
  ConfigMap appears, decompresses, and flips a tcpListener to `up` when a
  tcpConnector is added.

Verify: `go test ./internal/qdr/... ./internal/routerstatus/... ./internal/kube/adaptor/...`; cluster
...
* Replace `internal/nonkube/flow/collector.go` with a publisher that uses
  `internal/routerstatus` against the local router (`runtime.GetLocalRouterAddress`
