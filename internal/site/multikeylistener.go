...
// entities needed to implement a MultiKeyListener with priority failover strategy.
...
func UpdateBridgeConfigForMultiKeyListenerWithHostAndPort(siteId string, mkl *skupperv2alpha1.MultiKeyListener, host string, port int, config *qdr.BridgeConfig) {
	...
	if mkl.Spec.Strategy.PriorityFailover != nil {
		numKeys := len(mkl.Spec.Strategy.PriorityFailover.RoutingKeys)
		for i, routingKey := range mkl.Spec.Strategy.PriorityFailover.RoutingKeys {
