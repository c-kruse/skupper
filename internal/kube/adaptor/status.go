...
import (
	...
	"github.com/skupperproject/skupper/pkg/skrouter/status"
...
func (p *StatusPublisher) ConfigUpdated(cm *corev1.ConfigMap, syncErr error) {
	...
	desired, err := config.Parse([]byte(data))
	if err != nil {
...
func (p *StatusPublisher) publishLoop(ctx context.Context, hostname string) {
	client := mgmt.NewClient(func(ctx context.Context) (*amqp.Conn, error) { return amqp.Dial(ctx, "amqp://localhost:5672", nil) })
	...
	go index.Run(ctx)
	builder := status.Builder{Client: client, Group: deploymentName(), Hostname: hostname}
	...
	for {
		...
		doc, err := builder.Build(ctx, client.Local(), desired, adaptor, applied, index)
		if err == nil {
